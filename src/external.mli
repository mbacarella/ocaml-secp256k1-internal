...
module Key : sig
  ...
  val write :
    ?compress:bool -> Context.t -> ?pos:int -> Bigstring.t -> _ t -> int
...
module Sign : sig
  ...
  val write :
    ?der:bool -> Context.t -> Bigstring.t -> _ t -> (int, string) result
  ...
  val sign :
    Context.t -> sk:Key.secret Key.t -> Bigstring.t -> (plain t, string) result
  ...
  val sign_recoverable_exn :
    Context.t -> sk:Key.secret Key.t -> Bigstring.t -> recoverable t
  ...
  val write_sign_exn :
    Context.t -> Bigstring.t -> sk:Key.secret Key.t -> msg:Bigstring.t -> int
  ...
  val write_sign_recoverable_exn :
    Context.t -> sk:Key.secret Key.t -> msg:Bigstring.t -> Bigstring.t -> int

  (** {4 Verification} *)

  val verify_exn :
    Context.t -> pk:Key.public Key.t -> msg:Bigstring.t -> signature:_ t -> bool
  ...
  val recover_exn :
    Context.t -> signature:recoverable t -> Bigstring.t -> Key.public Key.t
