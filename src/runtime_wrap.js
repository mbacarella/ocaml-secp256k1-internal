...
function caml_secp256k1_ec_privkey_tweak_add(ctx, sk, tweak) {
  return call_wasm("_secp256k1_ec_privkey_tweak_add", ctx, sk, tweak) ? 1 : 0;
}
...
function caml_secp256k1_ec_pubkey_create(ctx, buf, sk) {
  return call_wasm("_secp256k1_ec_pubkey_create", ctx, buf, sk) ? 1 : 0;
}
...
function caml_secp256k1_ec_pubkey_serialize(ctx, buf, pk) {
  ...
  var flags = size == 33 ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED;

  var size_ba = wasm_int_pointer(size);
  call_wasm("_secp256k1_ec_pubkey_serialize", ctx, buf, size_ba, pk, flags);
...
function caml_secp256k1_ec_pubkey_parse(ctx, buf, pk) {
  ...
  return call_wasm("_secp256k1_ec_pubkey_parse", ctx, buf, pk, pk_len) ? 1 : 0;
}
...
function caml_secp256k1_ec_pubkey_tweak_add(ctx, pk, tweak) {
  return call_wasm("_secp256k1_ec_pubkey_tweak_add", ctx, pk, tweak) ? 1 : 0;
}
//Provides: caml_secp256k1_ec_pubkey_tweak_mul
//Requires: call_wasm
function caml_secp256k1_ec_pubkey_tweak_mul(ctx, pk, tweak) {
  return call_wasm("_secp256k1_ec_pubkey_tweak_mul", ctx, pk, tweak) ? 1 : 0;
}
...
function caml_secp256k1_ecdsa_signature_parse_compact(ctx, buf, sig) {
  return call_wasm("_secp256k1_ecdsa_signature_parse_compact", ctx, buf, sig)
    ? 1
    : 0;
}
...
function caml_secp256k1_ecdsa_signature_normalize(ctx, buf, sig) {
  return call_wasm("_secp256k1_ecdsa_signature_normalize", ctx, buf, sig)
    ? 1
    : 0;
}
...
function caml_secp256k1_ecdsa_verify(ctx, pubkey, msg, signature) {
  return call_wasm("_secp256k1_ecdsa_verify", ctx, signature, msg, pubkey)
    ? 1
    : 0;
}
...
function caml_secp256k1_ecdsa_sign(ctx, buf, seckey, msg) {
  return call_wasm("_secp256k1_ecdsa_sign", ctx, buf, msg, seckey, null, null)
    ? 1
    : 0;
}
...
//Requires: call_wasm
function caml_secp256k1_ecdsa_recoverable_signature_parse_compact(
  ctx,
  buf,
  signature,
  recid
...
//Requires: call_wasm
function caml_secp256k1_ecdsa_recoverable_signature_convert(
  ctx,
  buf,
  signature
) {
...
function caml_secp256k1_ecdsa_recover(ctx, buf, signature, msg) {
  return call_wasm("_secp256k1_ecdsa_recover", ctx, buf, signature, msg)
    ? 1
    : 0;
}
