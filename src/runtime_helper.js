...
function call_wasm() {
	...
	if(typeof x == "number" || typeof x == "boolean" || x === null){
	    ...
	    case 1: M.HEAPU8.set(x.data, p); break;
	    case 4: M.HEAPU32.set(x.data, p >> 2); break;
	    default: caml_failwith("call_wasm: unsuported buffer" + x.data.BYTES_PER_ELEMENT)
	    }
	    argsc[i] = p;
	}
	else {
	    caml_failwith("call_wasm: " + f + " called with unsupported argument type");
	}
    }
	...
	if(x instanceof Ml_Bigarray) {
	    var p = argsc[i];
	    switch(x.data.BYTES_PER_ELEMENT){
	    case 1: x.data.set(new g.Uint8Array(M.HEAPU8.buffer, p, x.data.length)); break;
	    case 4: x.data.set(new g.Uint32Array(M.HEAPU32.buffer, p, x.data.length)); break;
	    default: caml_failwith("call_wasm: unsuported buffer")
	    }
	    M._free(p);
	}
    }
...
function secp256k1_wasm_update(x, ptr) {
  var M = joo_global_object._SECP256K1;
  var data = new joo_global_object.Uint8Array(M.HEAPU8.buffer, ptr, x.data.length);
  x.data.set(data)
  M._free(ptr);
  return 0;
}
