module Num = struct
  ...
  external size : unit -> int = "sizeof_secp256k1_num" [@@noalloc]

  external copy : t -> t -> unit = "ml_secp256k1_num_copy" [@@noalloc]

  external get_bin : Cstruct.buffer -> int -> t -> unit
    = "ml_secp256k1_num_get_bin"
    [@@noalloc]

  external set_bin : t -> Cstruct.buffer -> int -> unit
    = "ml_secp256k1_num_set_bin"
    [@@noalloc]

  external mod_inverse : t -> t -> t -> unit = "ml_secp256k1_num_mod_inverse"
    [@@noalloc]

  external jacobi : t -> t -> int = "ml_secp256k1_num_jacobi" [@@noalloc]

  external compare : t -> t -> int = "ml_secp256k1_num_cmp" [@@noalloc]

  external equal : t -> t -> bool = "ml_secp256k1_num_eq" [@@noalloc]

  external add : t -> t -> t -> unit = "ml_secp256k1_num_add" [@@noalloc]

  external sub : t -> t -> t -> unit = "ml_secp256k1_num_sub" [@@noalloc]

  external mul : t -> t -> t -> unit = "ml_secp256k1_num_mul" [@@noalloc]

  external modulo : t -> t -> unit = "ml_secp256k1_num_mod" [@@noalloc]

  external shift : t -> int -> unit = "ml_secp256k1_num_shift" [@@noalloc]

  external is_zero : t -> bool = "ml_secp256k1_num_is_zero" [@@noalloc]

  external is_one : t -> bool = "ml_secp256k1_num_is_one" [@@noalloc]

  external is_neg : t -> bool = "ml_secp256k1_num_is_neg" [@@noalloc]

  external negate : t -> unit = "ml_secp256k1_num_negate" [@@noalloc]

  let size = size ()

  let get_bin cs = Cstruct.(get_bin (to_bigarray cs) (length cs))

  let set_bin r cs = Cstruct.(set_bin r (to_bigarray cs) (length cs))
...
module Scalar = struct
  ...
  external sqr : t -> t -> unit = "ml_secp256k1_scalar_sqr" [@@noalloc]
  ...
  external get_num : Num.t -> t -> unit = "ml_secp256k1_scalar_get_num"
    [@@noalloc]

  external order_get_num : Num.t -> unit = "ml_secp256k1_scalar_order_get_num"
    [@@noalloc]
...
module Field = struct
  ...
  external sqr : t -> t -> unit = "ml_secp256k1_fe_sqr" [@@noalloc]

  external sqrt : t -> t -> int = "ml_secp256k1_fe_sqrt" [@@noalloc]

  external is_quad_var : t -> bool = "ml_secp256k1_fe_is_quad_var" [@@noalloc]

  external inv : t -> t -> unit = "ml_secp256k1_fe_inv" [@@noalloc]

  external inv_var : t -> t -> unit = "ml_secp256k1_fe_inv_var" [@@noalloc]

  external inv_all_var : t -> Cstruct.buffer -> int -> unit
    = "ml_secp256k1_fe_inv_all_var"
    [@@noalloc]
  ...
  let inv_all_var r fes =
    ...
    List.iteri
      (fun i fe -> Cstruct.(blit (of_bigarray fe) 0 cs (i * size) size))
      fes ;
...
module Group = struct
  ...
  module Jacobian = struct
    ...
    external has_quad_y_var : t -> bool = "ml_secp256k1_gej_has_quad_y_var"
      [@@noalloc]

    external double_nonzero : t -> t -> Field.t option -> unit
      = "ml_secp256k1_gej_double_nonzero"
      [@@noalloc]
  ...
  external set_xquad : t -> Field.t -> unit = "ml_secp256k1_ge_set_xquad"
    [@@noalloc]
